...
# if you saved the results in diifferent folders, you can join them here
results_paths = [
    "../results_llms",
    "../results_mls",
]
    ...
    for model in models:
        for fold in folds:
            for results_path in results_paths:
                if 'GPT' in model:
                    globby = glob(f"{results_path}/{target}/{model}/*_{fold}_{scaled_data}_{few_feature}_{prompt_richness}.json")
                else:
                    globby = glob(f"{results_path}/{target}/{model}/*_{fold}_{scaled_data}_{False}_True.json")
                for file in globby:
